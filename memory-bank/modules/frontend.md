...
## 2. Estructura real
- `src/main.jsx`: providers globales, i18n, analytics y service worker en produccion.
- `src/App.jsx`: router, lazy loading, rutas publicas/privadas y tracking de creator query.
...
- `src/services/`: API, Supabase directo, cache y logica de dominio.
- `src/config/`: API URL, storage keys y gamificacion.
- `src/context/`: Auth y tema.

## 3. Flujo de datos
El usuario autentica con Supabase; `apiClient` anade Bearer token a llamadas Express. Algunos dominios leen Supabase directo desde frontend con RLS (`profiles`, progreso, rutinas, sesiones). Los servicios mantienen caches en localStorage/sessionStorage para recuperacion y fallback.
...
- Paginas lazy-loaded desde `src/App.jsx`.
...
Cambiar rutas rompe enlaces publicos y navegacion. Cambiar claves de localStorage o shape de respuestas rompe recuperacion offline/parcial. Activar `VITE_PROFILE_API_ENABLED` sin backend `/profiles/:userId` confirmado rompe perfil.
