...
## 1. Propósito
Registrar progreso corporal, resumir actividad del usuario y calcular indicadores de gamificación a partir de comidas, check-ins, dietas y entrenos.
...
3. También carga comidas, check-ins, dietas y sesiones de entrenamiento para resumen.
...
Tabla `progress_logs`: `id`, `user_id`, `peso`, `nota`, `created_at`. La tabla aparece en la auditoría RLS de `supabase/migrations/001_rls_security_audit.sql` y esas columnas se leen o escriben en `src/services/progressService.js`.

También lee `meal_analyses`, `checkins`, `diet_plans` y `workout_sessions`.

## 9. IA
No usa Gemini directamente.

## 10. Límites free/premium
No hay límites free/premium confirmados para crear logs de progreso. La puntuación XP confirmada incluye comida 10, entreno 15, check-in 20 y proteína 5.
...
El resumen depende de varias fuentes y caches locales; cambios en claves de storage o forma de datos pueden producir métricas inconsistentes.
...
## 14. Pendientes
No se confirmó endpoint backend para borrar o editar `progress_logs`; verificar antes de documentarlo como API.
